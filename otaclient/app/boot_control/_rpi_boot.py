...
class _RPIBootControl:
    ...
    def _update_firmware(self):
        ...
        try:
            ...
            # cleanup bak files generated by flash-kernel script as we actually don't use those files
    ...
    def finalize_switching_boot(self) -> bool:
        ...
        NOTE: we use a SWITCH_BOOT_FLAG_FILE to distinguish which reboot we are right now.
              If SWITCH_BOOT_FLAG_FILE presented, we know that we are at post_firmware_update,
              skip the finalization and directly return True.
              Otherwise, we know that we are at first reboot after switching boot,
              do the finalizaiton and firmware update, set the flag file and then reboot.
        ...
        try:
            ...
            # skip finalization and return True
