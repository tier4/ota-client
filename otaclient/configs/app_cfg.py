...
"""otaclient runtime configs and constants.

There are three types of configs defined in this module:
1. fixed internal configs: static configs value, should not be changed
    due to compatibility reason.
2. configurable configs(via env): configurable configs that changes
    otaclient's runtime behavior, check _ConfigurableConfig and _init_config
    for more details.
3. dynamically generated configs(paths consts): dynamically generated
    otaclient runtime configs based on configurable configs.
"""
...
def _init_config() -> Config:
    class _ConfigureViaENV(BaseConfigurableConfig, _ConfigurableConfig):
