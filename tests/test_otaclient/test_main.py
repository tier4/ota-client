...
class TestMain:
    ...
    @patch("otaclient._logging.configure_logging")
    def test_main_abort_writes_status_to_correct_directory(
        self, mock_logging, mocker: pytest_mock.MockerFixture, tmp_path
    ):
        """Test that abort writes ABORTED status to the correct directory from shared memory."""
        # Mock the modules and functions imported in main()
        mocker.patch("otaclient._otaproxy_ctx.otaproxy_control_thread")
        mocker.patch("otaclient.grpc.api_v2.main.grpc_server_process")
        mocker.patch("otaclient.ota_core.ota_core_process")
        ...
        # Create properly configured mock events for AbortOTAFlag
        # First call returns False (not set), second call returns True (abort requested)
        mock_shutdown_requested = MagicMock()
        mock_shutdown_requested.is_set.side_effect = [False, True, True]
        mock_reject_abort = MagicMock()
        mock_reject_abort.is_set.return_value = False
        ...
        # Create a test OTA status directory
        test_ota_status_dir = tmp_path / "ota-partition.a"
        test_ota_status_dir.mkdir(parents=True)

        # Mock SharedOTAClientStatusReader to return ota_status_dir
        mock_shm_reader = mocker.MagicMock()
        mock_status = mocker.MagicMock()
        mock_status.ota_status_dir = str(test_ota_status_dir)
        mock_shm_reader.sync_msg.return_value = mock_status
        mocker.patch(
            f"{MAIN_MODULE}.SharedOTAClientStatusReader", return_value=mock_shm_reader
        )
        ...
        # Verify ABORTED status was written to the correct directory
        status_file = test_ota_status_dir / "status"
        assert status_file.exists()
        assert status_file.read_text() == "ABORTED"

    @patch("otaclient._logging.configure_logging")
    def test_main_abort_falls_back_to_default_directory(
        self, mock_logging, mocker: pytest_mock.MockerFixture, tmp_path
    ):
        """Test that abort falls back to default directory when ota_status_dir is not in shared memory."""
        # Mock the modules and functions imported in main()
        mocker.patch("otaclient._otaproxy_ctx.otaproxy_control_thread")
        mocker.patch("otaclient.grpc.api_v2.main.grpc_server_process")
        mocker.patch("otaclient.ota_core.ota_core_process")
        ...
        mock_shm_reader = mocker.MagicMock()
        mock_status = mocker.MagicMock()
        mock_status.ota_status_dir = ""  # Empty - should fall back to default
        mock_shm_reader.sync_msg.return_value = mock_status
        mocker.patch(
            f"{MAIN_MODULE}.SharedOTAClientStatusReader", return_value=mock_shm_reader
        )
        ...
        # Verify ABORTED status was written to the default directory
        status_file = default_ota_status_dir / "status"
        assert status_file.exists()
        assert status_file.read_text() == "ABORTED"
