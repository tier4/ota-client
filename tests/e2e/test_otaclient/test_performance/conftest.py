...
fixtures) lives in the top-level `tests/conftest.py`; only the
performance-report machinery and the dummy boot controller are local here.
"""
...
def _ota_image_servers(
    legacy_ota_image_server: str, ota_image_v1_server: str
) -> Generator[None]:
    """Auto-start the OTA-image HTTP servers for performance tests.

    The performance test addresses the servers via the hardcoded
    `OTA_IMAGE_URL` / `OTA_IMAGE_V1_URL` constants rather than requesting the
    fixtures, so this autouse fixture wires up the shared session-scoped server
    fixtures (defined in `tests/conftest.py`) for the whole session.
    """
    yield
