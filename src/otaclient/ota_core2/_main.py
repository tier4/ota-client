...
"""Main entrypoint of ota_core process."""
...
from ._rollbacker import OTARollbackImpl
from ._updater import OTAUpdateImpl
...
class OTAClient:
        ...
        *,
        ...
        try:
            ...
            _err_msg = (
                ...
                "OTA will NOT occur on no CA chains installed!!!"
            )
            logger.error(_err_msg)
            self.ca_chains_store = CACertStore()

        # NOTE: for legacy OTA image
        self.ca_chains_store_with_prefix = None
        try:
            self.ca_chains_store_with_prefix = load_ca_cert_chains_with_prefix(
                cfg.CERT_DPATH
            )
        except CACertStoreInvalid as e:
            _err_msg = (
                f"failed to import ca_chains_store: {e!r}, OTA with legacy OTA image"
                "will NOT occur on no CA chains installed!!!"
            )
            logger.error(_err_msg)
            self.ca_chains_store_with_prefix = CAChainStoreWithPrefix()
    ...
    def rollback(self, request: RollbackRequestV2) -> None:
        ...
        try:
            ...
            self._live_ota_status = OTAStatus.ROLLBACKING
            OTARollbackImpl(boot_controller=self.boot_controller).execute()
        except ota_errors.OTAError as e:
            self._on_failure(
                e,
                ota_status=OTAStatus.FAILURE,
