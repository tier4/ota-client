    ...
    def _load_status_file(self):
        ...
        if _loaded_ota_status is None:
            logger.info(
                ...
                f"initializing and set/store status to {OTAStatus.INITIALIZED}..."
            )
            self._store_current_status(OTAStatus.INITIALIZED)
            self._ota_status = OTAStatus.INITIALIZED
            return
        logger.info(f"status loaded from file: {_loaded_ota_status}")
        ...
        if self._is_switching_boot(self.active_slot):
            ...
            logger.error(
                f"we are in {_loaded_ota_status} ota_status, "
