    ...
    def execute(self) -> None:
        ...
        try:
            ...
            with self._abort_handler.critical_zone():
                self._pre_update()
                logger.warning("TEST CODE: 120 second sleep in critical zone after pre_update")
                time.sleep(120)
            ...
            logger.warning("TEST CODE: sleep BEFORE entering final phase")
            time.sleep(120)
            self._abort_handler.enter_final_phase()
            logger.warning("TEST CODE: 120 second sleep AFTER entering final phase")
            time.sleep(120)
            self._post_update()
...
class OTAUpdaterForOTAImageV1(OTAImageV1SupportMixin, OTAUpdaterBase):
    ...
    def _process_metadata(self, only_metadata_verification: bool = False) -> None:
        ...
        # for Jetson UEFI device, apply BSP check, and interrupt OTA if needed
        if isinstance(self._boot_controller, JetsonUEFIBootControl) and (
            _ota_image_bsp_ver := self._ota_image_helper.bsp_version_str
        ):
            self._nvidia_jetson_check_bsp(_ota_image_bsp_ver)
