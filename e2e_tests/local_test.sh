...
setup_test_environment() {
    ...
    sudo cp -a "$REPO_LOCATION" "$WORKING_DIR"
    ...
    sudo docker create --name $CONTAINER_NAME base-image
    sudo docker export $CONTAINER_NAME > ./base-image.tgz
...
do_e2e_test() {
    ...
    python3 -m pip install -r ./"$REPO"/e2e-tests/requirements.txt
