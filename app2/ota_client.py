...
def _cp_preserve(src: Path, dst_dir: Path):
...
def _cp_recursive(src: Path, dst_dir: Path):
    _cp_preserve(src, dst_dir)
    if src.is_dir() and not src.is_symlink():
        for src_child in src.glob("*"):
            _cp_recursive(src_child, dst_dir / src.name)


def cp_parents(src: Path, dst_dir: Path):
    dst_path = dst_dir
    ...
    for parent in reversed(list(src.parents)):
        _cp_preserve(parent, dst_path)
        dst_path = dst_path / parent.name
    _cp_recursive(src, dst_path)
